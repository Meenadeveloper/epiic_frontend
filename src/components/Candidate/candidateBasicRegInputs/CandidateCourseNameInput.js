...
function CandidateCourseNameInput({
  name,
  formData,
  formErrors,
  handleChange,
     ...
     const [selectedOption, setSelectedOption] = useState(
       formData.courseName ? { label: formData.courseName, value: formData.courseId } : null
     );
     ...
     const handleSelectChange = (option) => {
       ...
       handleChange({
         target: { name: 'courseId', value: option.value }, // Set college ID
       });
     };
  ...
  return (
    <>
      ...
      <label className="register-label">Course Name</label>
      <Select
        isClearable
...
{formErrors[name] && <p className="error">{formErrors[name]}</p>}    </div> 
    </>
  )
}

export default CandidateCourseNameInput