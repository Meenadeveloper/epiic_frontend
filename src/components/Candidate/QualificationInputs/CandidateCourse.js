...
function CandidateCourse() {
  ...
  const fetchData = async () => {
    try {
      const url = `${process.env.REACT_APP_CANDIDATE_BASE_URL}/get-courses?qualification_id=${formData.qualificationId}`;
        if (formData.qualificationId) {
        const response = await axios.get(url);
        // Assuming the qualifications are in response.data.qualification
        const optionDatas = response.data.courses || [];
      ...
      const transformedData = optionDatas.map(optionData => ({
        ...
        label: optionData.course_name,  // option label is the department_name
      }));
 ...
 useEffect(() => {
    fetchData();
  }, [formData.qualificationId]);  // Empty dependency array means this runs once after the initial render
  ...
  const handleChange = (selectedOption) => {
    setSelectedOption(selectedOption);  // Update selected option in state
     ...
     onHandleChange('course', selectedOption ? selectedOption.label : '');
     onHandleChange('courseId', selectedOption ? selectedOption.value : '');
  };
...
<div className="register-form-control">
        <label className="register-label">Course</label>
