...
function CandidateState() {
  ...
  const fetchData = async () => {
    try {
        const response = await axios.get(process.env.REACT_APP_STATE_API_URL);
         // Assuming the qualifications are in response.data.qualification
        const optionDatas = response.data.states || [];
      ...
      const transformedData = optionDatas.map(optionData => ({
        ...
        label: optionData.state_name,  // option label is the department_name
      }));
 ...
 useEffect(() => {
  ...
  const handleChange = (selectedOption) => {
    setSelectedOption(selectedOption);  // Update selected option in state
     ...
     onHandleChange('state', selectedOption ? selectedOption.label : '');
     onHandleChange('stateId', selectedOption ? selectedOption.value : '');
  };
