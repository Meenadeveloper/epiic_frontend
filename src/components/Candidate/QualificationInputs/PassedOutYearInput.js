...
function PassedOutYearInput() {

    const { onHandleChange  } = useQualification();
  ...
  const fetchData = async () => {
    try {
        const response = await axios.get(process.env.REACT_APP_CANDIDATE_GET_DATA_API);
         // Assuming the qualifications are in response.data.qualification
        const optionDatas = response.data.passoutYear || [];
         // Transform qualifications into the format required by react-select
      const transformedData = optionDatas.map(optionData => ({
        value: optionData,  // option value is the id
        label: optionData,  // option label is the department_name
      }));
 ...
 useEffect(() => {
  ...
  const handleChange = (selectedOption) => {
    setSelectedOption(selectedOption);  // Update selected option in state
     ...
     onHandleChange('passout_year', selectedOption ? selectedOption.label : '');
     onHandleChange('passout_year', selectedOption ? selectedOption.value : '');
  };
 ...
 <div className="register-form-control">
        <label className="register-label">Passout Year</label>
...
export default PassedOutYearInput