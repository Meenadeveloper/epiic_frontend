...
function QualificationInput() {
  ...
  const fetchData = async () => {
    try {
        ...
        const optionDatas = response.data.qualification || [];
      ...
      const transformedData = optionDatas.map(optionData => ({
        ...
        label: optionData.department_name,  // option label is the department_name
      }));
 ...
 useEffect(() => {
  ...
  const handleChange = (selectedOption) => {
    setSelectedOption(selectedOption);  // Update selected option in state
     ...
     onHandleChange('qualification', selectedOption ? selectedOption.label : '');
     onHandleChange('qualificationId', selectedOption ? selectedOption.value : '');
  };
