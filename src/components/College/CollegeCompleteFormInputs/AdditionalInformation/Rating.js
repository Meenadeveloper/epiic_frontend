...
function Rating({ additionalData, formErrors, handleChange,  }) {
  // States for selected option, input value, and API data
        const [selectedOption, setSelectedOption] = useState(
          additionalData.rating_with_year
            ? { label: additionalData.rating_with_year, value: additionalData.rating_with_year_id }
            : null
        );
        ...
        useEffect(() => {
          const apiUrl = process.env.REACT_APP_COLLEGE_GET_API;
          ...
          fetch(apiUrl)
            ...
            .then((data) => {
              ...
              setTurnoverRanges(data.ratings); // Update state with fetched data
            })
            .catch((error) => {
        ...
        const handleSelectChange = (option) => {
          ...
          if (!option) {
            ...
            setInputValue('');
            handleChange({
              target: { name: 'rating_with_year', value: '' },
            });
            handleChange({
              target: { name: 'rating_with_year_id', value: '' },
            });
            return;
          }
        ...
        const handleCreateOption = (inputValue) => {
         ...
         handleChange({
          target: { name: 'rating_with_year', value: newOption.label },
        });
      
        handleChange({
          target: { name: 'rating_with_year_id', value: newOption.value },
        });
        };
        ...
        const selectOptions = turnoverRanges.map((range) => ({
          ...
          label: String(range.rating), // Concatenate as a string
        }));
  ...
  return (
    ...
    <>
     <div className="register-form-control">
      <label className="register-label">Rating</label>
      <CreatableSelect
  isClearable
...
/>
      ...
      {formErrors.rating_with_year_id && <p className="error">{formErrors.rating_with_year_id}</p>}
    </div>
    </>
  )
}
