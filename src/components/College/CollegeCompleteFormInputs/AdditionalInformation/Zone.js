...
function Zone({ additionalData, formErrors, handleChange }) {
 // States for selected option, input value, and API data
      const [selectedOption, setSelectedOption] = useState(
        additionalData.zone
          ? { label: additionalData.zone, value: additionalData.zoneId }
          : null
      );
      ...
      const handleCreateOption = (inputValue) => {
       ...
       handleChange({
        target: { name: 'zone', value: newOption.label },
      });
    
      handleChange({
        target: { name: 'zoneId', value: newOption.value },
      });
      };
  ...
  return (
    <>
      ...
      <label className="register-label">Zone</label>
      <CreatableSelect
  isClearable
...
/>
      ...
      {formErrors.zone && <p className="error">{formErrors.zone}</p>}
    </div>
    </>
  )
}
