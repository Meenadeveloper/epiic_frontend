...
function Accreditation({ additionalData, formErrors, handleChange }) {
   // States for selected option, input value, and API data
         const [selectedOption, setSelectedOption] = useState(
           additionalData.accreditation
             ? { label: additionalData.accreditation, value: additionalData.accreditationId }
             : null
         );
         ...
         useEffect(() => {
           ...
           fetch(apiUrl)
             ...
             .then((data) => {
               ...
               setTurnoverRanges(data.zone); // Update state with fetched data
             })
             .catch((error) => {
         ...
         const handleSelectChange = (option) => {
           ...
           if (!option) {
             ...
             setInputValue('');
             handleChange({
               target: { name: 'accreditation', value: '' },
             });
             handleChange({
               target: { name: 'accreditationId', value: '' },
             });
             return;
           }
         ...
         const handleCreateOption = (inputValue) => {
          ...
          handleChange({
           target: { name: 'accreditation', value: newOption.label },
         });
       
         handleChange({
           target: { name: 'accreditationId', value: newOption.value },
         });
         };
         ...
         const selectOptions = turnoverRanges.map((range) => ({
           ...
           label: String(range.zone_name), // Concatenate as a string
         }));

  return (
    <>
<div className="register-form-control">
      <label className="register-label">Accreditation</label>
      <CreatableSelect
  isClearable
...
/>
      ...
      {formErrors.accreditationId && <p className="error">{formErrors.accreditationId}</p>}
    </div>
    </>
  )
}
