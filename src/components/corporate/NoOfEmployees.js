...
function NoOfEmployees({
   ...
   // States for selected option, input value, and API data
        const [selectedOption, setSelectedOption] = useState(
          formData.turnover
            ? { label: formData.noofemployees, value: formData.noofemployeesId }
            : null
        );
        ...
        useEffect(() => {
          ...
          fetch(apiUrl)
            ...
            .then((data) => {
              ...
              setTurnoverRanges(data.employeeRange); // Update state with fetched data
            })
            .catch((error) => {
        ...
        const handleSelectChange = (option) => {
          ...
          if (!option) {
            ...
            setInputValue('');
            handleChange({
              target: { name: 'noofemployees', value: '' },
            });
            handleChange({
              target: { name: 'noofemployeesId', value: '' },
            });
            return;
          }
        ...
        const handleCreateOption = (inputValue) => {
         ...
         handleChange({
          target: { name: 'noofemployees', value: newOption.label },
        });
      
        handleChange({
          target: { name: 'noofemployeesId', value: newOption.value },
        });
        };
        ...
        const selectOptions = turnoverRanges.map((range) => ({
          ...
          label: `${range.employee_range}`, // Concatenate as a string
        }));
