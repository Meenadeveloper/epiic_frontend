...
function NatureOfIndustry({ formData, formErrors, handleChange }) {
   // States for selected option, input value, and API data
    const [selectedOption, setSelectedOption] = useState(
      formData.natureofindustry
        ? [{ label: formData.natureofindustry, value: formData.natureofindustryId }] // Ensure it's an array for multi-select
        : []
    );
 ...
 useEffect(() => {
  ...
  const handleSelectChange = (options) => {
    ...
    handleChange({
      target: { name: 'natureofindustry', value: selectedValues.join(', ') }, // Pass comma-separated values
    });
    handleChange({
      target: { name: 'natureofindustryId', value: selectedValues.join(', ') },
    });
  };
  ...
  const handleCreateOption = (inputValue) => {
    ...
    handleChange({
      target: { name: 'natureofindustry', value: [...selectedOption, newOption].map(option => option.label).join(', ') },
    });
    handleChange({
      target: { name: 'natureofindustryId', value: [...selectedOption, newOption].map(option => option.value).join(', ') },
    });
  };
...
// Custom DropdownIndicator with SVG
  ...
  const selectOptions = turnoverRanges.map((range) => ({
    ...
    label: `${range.industry_name}`, // Concatenate as a string
  }));
  ...
  return (
    <div className="register-form-control">
      ...
      {formErrors.natureofindustry && <p className="error">{formErrors.natureofindustry}</p>}
    </div>
  );
}

export default NatureOfIndustry;
